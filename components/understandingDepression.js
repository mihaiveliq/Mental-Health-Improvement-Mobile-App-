...
export default function UnderstandingBadMoods({navigation}) {
    ...
    const colRef = collection(db, 'understandingBadMoods')
    useEffect(
        () => {
...
function ParagraphForm({initialText, textId, handleDoneEdit}) {
    ...
    return (
        <Formik
            ...
            onSubmit={(values) => {
                const paragraphRef = doc(db, 'understandingBadMoods', textId);
                setText(values);
